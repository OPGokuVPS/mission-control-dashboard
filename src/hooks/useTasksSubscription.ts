            ...
            .on(
                ...
                (payload) => {
                    ...
                    switch (type) {
                        case 'INSERT':
                        case 'UPDATE': {
                            // Upsert single record by replacing it in every query
                            // that holds the full list.
                            invalidateRef.current({
                                queryKey: QUERY_KEYS.tasks.all,
                            });
                            invalidateRef.current({
                                queryKey: QUERY_KEYS.task.byId(record!.id),
                            });
                            break;
                        }
                        case 'DELETE': {
                            ...
                            invalidateRef.current({
                                queryKey: QUERY_KEYS.task.byId(oldId! as number),
                            });
                            break;
                        }
                    }
